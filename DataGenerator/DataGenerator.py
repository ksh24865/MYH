...
# sensor : [1(udust), 2(dust), 3(humid),4(lux)(조도,오류체크),5(visitor)], [6(shumid)], , [7(trash)],  
...
for node in range(3):
    ...
    for sink in range(11):
        ...
        for sensor in range(1,6):
            data = {"sensor_id":sensor,"node_id": 9*sink+3*node+1,"values":[nowline[sensor]],"timestamp":timestamp}
            func.produce_data(producer,data,conf.topic)        
        sensor=6
        data = {"sensor_id":sensor,"node_id": 9*sink+3*node+2,"values":[nowline[sensor]],"timestamp":timestamp}
        func.produce_data(producer,data,conf.topic)
        sensor=7
        data = {"sensor_id":sensor,"node_id": 9*sink+3*node+3,"values":[nowline[sensor]],"timestamp":timestamp}
        func.produce_data(producer,data,conf.topic)
    # time.sleep(30)

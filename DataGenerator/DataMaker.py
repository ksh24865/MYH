import pandas as pd
import numpy
# udust,dust,humid,lux,shumid,visitor,trash
# sensor : [1(udust), 2(dust), 3(temp),3(humid),4(lux)(조도,오류체크),5(visitor)], [6(shumid)], , [7(trash)],  
#기상정보
...
air_df = air_df[['초미세먼지(㎍/㎥)','미세먼지(㎍/㎥)','기온(℃)','상대습도( %)','조도(lux)']] # 필요한 것만 인덱싱
...
#df 합친 후 저장
result = pd.concat([air_df,visitor_df,soil_humid,trash_df],axis=1, join='inner') # 데이터 모두 합치기
result.columns=["udust","dust","temper","humid","lux","visitor","shumid","trash"] # col 이름 변경
